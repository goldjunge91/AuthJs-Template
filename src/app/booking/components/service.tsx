...
export function PackageSelection() {
  ...
  //           <Card
  //             key={pkg.id}
  //             onClick={() => handle-package-select(pkg.id)}
  //             className={cn(
  //               'relative cursor-pointer transition-all',
  //               selected-package === pkg.id && 'border-primary bg-primary/5'
  //             )}
  //           >
  //             <CardHeader>
  ...
  //                 {pkg.price[vehicle-class]}
  //               </div>
  //               <div className='text-sm text-muted-foreground'>
  //                 Dauer: {pkg.duration[vehicle-class]} Minuten
  //               </div>
...
//     vehicle-class,
//     selected-package,
//     additional-options: selected-options,
//     set-additional-options,
//     set-duration,
//   } = useBookingStore();

//   if (!vehicle-class || !selected-package) return null;

//   // Nur Zusatzoptionen anzeigen, die zum gewählten Paket passen
//   const available-options = additionalOptions.filter(option =>
//     option.package-id?.includes(selected-package)
//   );

//   const handle-option-toggle = (option-id) => {
//     const option = available-options.find(opt => opt.id === option-id);
//     const new-options = selected-options.includes(option-id)
//       ? selected-options.filter(id => id !== option-id)
//       : [...selected-options, option-id];

//     set-additional-options(new-options);

//     // Aktualisiere die Gesamtdauer
//     if (option) {
//       const base-duration = packages.find(pkg => pkg.id === selected-package)?.duration[vehicle-class] || 0;
//       const additional-duration = new-options.reduce((total, id) => {
//         const opt = additionalOptions.find(o => o.id === id);
//         return total + (opt?.duration[vehicle-class] || 0);
//       }, 0);
//       set-duration(base-duration + additional-duration);
//     }
//   };
...
//         <Card
//           key={option.id}
//           onClick={() => handle-option-toggle(option.id)}
//           className={cn(
//             'relative cursor-pointer',
//             selected-options.includes(option.id) && 'border-primary'
//           )}
//         >
//           <CardContent>
//             <h3 className='font-medium'>{option.title}</h3>
...
//             <div className='mt-2'>
//               <span className='text-xl font-bold text-primary'>
//                 {option.price[vehicle-class]}
//               </span>
//               <span className='text-sm text-muted-foreground'>
//                 (+{option.duration[vehicle-class]} Min.)
//               </span>
//             </div>
//           </CardContent>
//         </Card>
